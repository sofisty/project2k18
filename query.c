...
void store_pred(char* pred_str, pred* p){
	...
	p->cols=malloc((strlen(predicate)-1)*sizeof(int)); //enw ta dots einai 2 ,  iparxei kai -1 pou apothikevetai sto telos opote desmevw strlen-1 
	if (p->cols== NULL) {
	    fprintf(stderr, "Malloc failed \n"); 
	    exit(-1);
	}
...
query* store_query(char* qstr, query* q){
	...
	q->rels=malloc(num_rels*sizeof(int)); //desmevw xwro gia ta relations tou query
	if (q->rels== NULL) {
	    fprintf(stderr, "Malloc failed \n"); 
	    exit(-1);
	}
	...
	q->preds=malloc(sizeof(pred)); //desmevw xwro gia thn lista twn kathgorhmatwn ths domhs query
	if (q->preds == NULL) {
	    fprintf(stderr, "Malloc failed \n"); 
	    exit(-1);
	}
	...
	predicates[0]=malloc((strlen(token)+1)*sizeof(char)); //desmevw xwro gia to prwto kathgorhma string
	...
	for(i=0;i<num_preds;i++){ //apokwdikopoiw kai apothikevw kathe kathgorhma sthn lista ths domhs query
		if(i==0){
			...
			curr=malloc(sizeof(pred)); //desmevw xwro gia to kathgorhma sth lista
			if (curr== NULL) {
			    fprintf(stderr, "Malloc failed \n"); 
			    exit(-1);
			}
